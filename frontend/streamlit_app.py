...
# Set page config for wide layout and Tron-inspired title
st.set_page_config(
    page_title="Zero Waste Energy Optimizer",
    page_icon="D:/zero-waste-energy-optimizer/frontend/public/assets/logo.png",
    layout="wide",
...
# Apply theme
# Assign color variables to avoid unterminated f-string expressions
...
st.markdown(f"""
    ...
    h1, h2, h3, h4, h5, h6 {{
        ...
        text-shadow: 0 0 10px {heading_shadow};
    }}
...
# Sidebar
with st.sidebar:
    st.image("D:/zero-waste-energy-optimizer/frontend/public/assets/store-icon.svg", width=50)
    page = st.selectbox("Navigate", [
        "Dashboard", "Customer App", "Analytics", "AI Agent Chat",
        "Sustainability Tracker", "Report Generator", "Blockchain Analytics"
    ], key="nav")
    theme = st.selectbox("Theme", ["Dark", "Bright"], key="theme_select")
    st.session_state.theme = theme.lower()
    st.markdown("---")
    st.markdown("**Zero Waste Energy Optimizer**")
    st.markdown("Join the sustainability revolution with AI and blockchain.")
    if sdg_animation:
        st_lottie(sdg_animation, height=100, width=100, key="sidebar_sdg_lottie")

# Header
st.markdown("""
    <div style="text-align: center; padding: 20px;">
        <img src="/assets/logo.png" alt="Zero Waste Energy Optimizer Logo" style="width: 80px; height: 80px; animation: spin 10s linear infinite;">
        <h1 style='font-size: 36px; margin: 10px 0;'>Zero Waste Energy Optimizer</h1>
        <p style="color: #00FF00; font-size: 20px; text-shadow: 0 0 5px #00FF00;">AI & Blockchain for a Sustainable Future</p>
    </div>
    <style>
        @keyframes spin {
...
""", unsafe_allow_html=True)
if lottie_animation:
    st_lottie(lottie_animation, height=100, width=100, key="header_lottie")
...
if page == "Dashboard":
    ...
    if lottie_animation:
        st_lottie(lottie_animation, height=100, width=100, key="dashboard_lottie")
    ...
    if not df.empty:
        ...
        st.warning("No inventory data available.")
    if carbon_animation:
        st_lottie(carbon_animation, height=100, width=100, key="customer_lottie")
    ...
    for vis in visualizations:
        with st.expander(vis["title"], expanded=False):
            ...
            try:
                if vis["type"] == "html":
                    components.iframe(vis["url"], height=700, scrolling=True)
                elif vis["type"] == "image":
    ...
    if lottie_animation:
        st_lottie(lottie_animation, height=100, width=100, key="analytics_lottie")
    ...
    if sdg_animation:
        st_lottie(sdg_animation, height=100, width=100, key="chat_lottie")
    ...
    if metrics:
        ...
        st.markdown(f"""
            <div style="text-align: center; font-size: 24px; color: #00FF00; text-shadow: 0 0 5px #00FF00;">
                {metrics.get('co2_reduction', 0):.2f} tons CO2 reduced
            </div>
        """, unsafe_allow_html=True)
        if carbon_animation:
            st_lottie(carbon_animation, height=100, width=100, key="co2_lottie")
    ...
    if sdg_animation:
        st_lottie(sdg_animation, height=100, width=100, key="sustainability_lottie")
    ...
    if lottie_animation:
        st_lottie(lottie_animation, height=100, width=100, key="report_lottie")
    ...
    if carbon_animation:
        st_lottie(carbon_animation, height=100, width=100, key="blockchain_lottie")

# Footer
st.markdown(f"""
    <div style="text-align: center; color: {'#00FFFF' if st.session_state.theme == 'dark' else '#008000'}; margin-top: 20px; text-shadow: 0 0 5px #00FF00;">
